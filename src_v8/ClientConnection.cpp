...
void ClientConnection::WaitForRequests() {
	...
	while(!parar)
	{
		...
		{
			/*Con el comando #ifdef podemos detectar el sistema operativo. En el caso
			de __linux__ detectamos si es un linux, en caso de serlo se ejecutaría
			esa parte del codigo, en caso de no se pasaría el siguiente if.*/
			#ifdef __linux__
	  ...
	  //Comando TYPE
		...
		else if (COMMAND("PASV"))
		{
			...
			//***** Sumamente Importante el salto de linea siguiente *****
      ...
      if(logged_in){
        ...
        struct dirent* arc_dir;
				...
				d = opendir(".");	//Ubicación actual

				if(d == NULL)
					fprintf(fd, "425 Can't open data connection.\n");
				else{
	        while( (arc_dir = readdir(d)) != NULL){
